...
use strict;
use Data::Dumper;
use IG::ClassAccessor; #XXXIG
#XXXIG use base qw(Class::Accessor);
...
function jsdebug(){
    ...
    for( var i=0; i < ajax.length; i++ ) {
      ...
      decodeURI(ajax[i].url) + ' <' + '/a' + '><' + 'br>'; //#XXXIG
    }
    document.getElementById('pjxdebugrequest').innerHTML = tmp + "</" + "pre" + ">"; //#XXXIG
}

EOT

    #XXXIG if ( $self->JSDEBUG() <= 1 ) {
