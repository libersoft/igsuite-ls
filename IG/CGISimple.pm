## IGSuite 4.0.0
## Procedure: CGISimple.pm
## Last update: 25/05/2009
#############################################################################
# IGSuite 4.0.0 - Provides an Office Suite by  simple web interface         #
# Copyright (C) 2002 Dante Ortolani  [LucaS]                                #
# Author of this module:                                                    #
# Copyright 2001 Dr James Freeman jfreeman@tassie.net.au                    #
#                                                                           #
...
use vars qw(
 ...
 $NPH $DEBUG $NO_NULL $FATAL *in $UPLOAD_HOOK
);#XXXIG added $UPLOAD_HOOK
...
BEGIN {
  ...
  if ( "\t" ne "\011" ) {
    eval { require IG::CGISimpleUtil }; #XXXIG
    if ( $@ ) {
      croak
...
# $fh will be undef so only do file stuff if $fh is true using $fh && syntax
  ...
  while ( $got_data < $length ) {
    ...
    #XXXIG Hook upload status
    &$UPLOAD_HOOK( $filename, $length, $got_data)
      if $filename && $UPLOAD_HOOK && ref($UPLOAD_HOOK) eq 'CODE';
  ...
  #XXXIG Hook upload status #XXX2TEST
  &$UPLOAD_HOOK( $filename, $length, $length)
    if $filename && $UPLOAD_HOOK && ref($UPLOAD_HOOK) eq 'CODE';
...
sub escapeHTML {
  ...
  require IG::CGISimpleUtil; #XXXIG
...
sub cookie {
  my ( $self, @params ) = @_;
  require IG::CGISimpleCookie; #XXXIG
  require IG::CGISimpleUtil; #XXXIG
...
sub multipart_init {
  my ( $self, @p ) = @_;
  use IG::CGISimpleUtil; CGI::Simple::Util->import( qw(rearrange) ); #XXXIG
