...
# Customized by Luca Dante Ortolani [LucaS]                                 #
#                                                                           #
...
%mailcfg = (
    ...
    'smtp'    => [ ($IG::smtp_conf{host} || 'localhost') ],

    'from'    => '', # default sender e-mail, used when no From header in mail

    'mime'    => 0, # use MIME encoding by default
    ...
    'port'    => $IG::smtp_conf{port}  || 25,
    'debug'   => $IG::smtp_conf{debug} || '0' ## prints stuff to STDERR
);
...
sub sendmail {
    ...
    $error = '';
    $log = "Sendmail v. $VERSION - "    . scalar(localtime()) . "\n";
    ...
    unless (@recipients) {
        return fail(" no recipient in [$recip]! ")
    }

    # get local hostname for polite HELO
    $localhost = $ENV{'SERVER_ADDR'} || 'localhost';
...
Sendmail contains mainly &sendmail, which takes a hash with the
...
C<perl -MCPAN -e "install Sendmail">
...
C<unshift @{$Sendmail::mailcfg{'smtp'}} , 'my.mail.server';>
...
C<sendmail(%mail) || print "Error sending mail: $Sendmail::error\n";>
...
C<$Sendmail::error> and C<$Sendmail::log>.
...
C<use Sendmail qw(sendmail $address_rx time_to_date);>

=head2 Sendmail::time_to_date()

convert time ( as from C<time()> ) to an RFC 822 compliant string for the
Date header. See also L<"%Sendmail::mailcfg">.

=head2 $Sendmail::error
...
(with something like C<use Sendmail qw(sendmail %mailcfg)>).
...
=item $Sendmail::default_smtp_server

=item $Sendmail::default_smtp_port

=item $Sendmail::default_sender

=item $Sendmail::TZ

=item $Sendmail::connect_retries

=item $Sendmail::retry_delay

=item $Sendmail::use_MIME
...
=head1 ANOTHER EXAMPLE
  ...
  print "Testing Sendmail version $Sendmail::VERSION\n";
  print "Default server: $Sendmail::mailcfg{smtp}->[0]\n";
  print "Default sender: $Sendmail::mailcfg{from}\n";
  ...
  %mail = (
      ...
      'X-Mailer' => "Sendmail version $Sendmail::VERSION",
  );
  ...
  # cheat on the date:
  $mail{Date} = Sendmail::time_to_date( time() - 86400 );

  if (sendmail %mail) { print "Mail sent OK.\n" }
  else { print "Error sending mail: $Sendmail::error \n" }
